...
public class HudControl : MonoBehaviour
{
    ...
    private void Awake()
    {
        // Cache and validate UI references to avoid NullReferenceExceptions
        Transform mod = transform.Find("Module");
        if (mod == null)
        {
            Debug.LogError("HudControl: missing child 'Module' in HUD hierarchy.");
            return;
        }

        // Subscribe to player events only if Player exists
